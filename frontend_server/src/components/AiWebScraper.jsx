...
const AiWebScraper = () => {
  ...
  const handleScrape = async (e) => {
    ...
    try {
      console.log(process.env.LLAMA_URL)
      const res = await api.post("http://20.223.114.24:80" + "/api/scrape/", { url });
  ...
  const handleParse = async (e) => {
    ...
    try {
      const res = await api.post("http://20.223.114.24:80" + "/api/parse/", {
  ...
  const presetValues = async (note) => {
    ...
    const res = await api.post("http://20.223.114.24:80" + "/api/get_dom_content/", {
      search_url: note.url,
    });
